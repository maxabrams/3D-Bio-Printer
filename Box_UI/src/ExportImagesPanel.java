...
public class ExportImagesPanel implements ActionListener {
	...
	public void showNameDialogBox(Dish dish) {
		...
		JOptionPane.showMessageDialog(frame, captureSpinner,
				"Enter Capture Rate", JOptionPane.PLAIN_MESSAGE);

		frame.dispose();

		dish.setCaptureRate(Integer
				.parseInt(captureModel.getValue().toString()));
	...
	public void dishSelected(Dish dish) {
		...
		timeStampString = new SimpleDateFormat("yyyy.MM.dd.HH.mm.ss")
				.format(new Date());
		// right now only save time- but probably need to save date too but it
		// won't let me save it all in an Int
		timeStamp = Integer.parseInt(new SimpleDateFormat("HHmmss")
				.format(new Date()));
